import { DocumentTypesDatabase, DocumentsDatabase, DocumentTypeRecord, DocumentRecord } from './database'
...
export async function createDocumentType(data: Omit<DocumentType, 'id' | 'created_at'>): Promise<DocumentType> {
...
export async function updateDocumentType(id: string, updates: Partial<DocumentType>): Promise<DocumentType | null> {
  const db = new DocumentTypesDatabase()
...
export async function getDocument(documentTypeId: string, docId: string): Promise<DocumentWithData | null> {
  const docsDb = new DocumentsDatabase()
...
export async function getDocumentFile(documentTypeId: string, docId: string): Promise<{ buffer: Buffer; filename: string } | null> {
  const docsDb = new DocumentsDatabase()
