...
export class DocumentTypesDatabase {
  ...
  async update(id: string, updates: Partial<DocumentTypeRecord>): Promise<DocumentTypeRecord | null> {
    return new Promise((resolve, reject) => {
      this.db.update(
        { id },
        { $set: updates },
...
export class DocumentsDatabase {
  ...
  async findByStatus(status: DocumentRecord['status'], documentTypeId?: string): Promise<DocumentRecord[]> {
